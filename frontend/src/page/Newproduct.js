...
const Newproduct = () => {
  ...
  return (
    <div className='p-4'>
      <form className='m-auto w-full max-w-md showdow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <div className='h-40 w-full bg-slate-200 rounded flex items-center justify-center'>
          {
            data.image ? <img src={data.image} className='h-full'/> : <span className='text-5xl'><FaCloudUploadAlt/></span>
          }
          <input type={'file'} accept='image/*' id='image' name='iamge' onChange={uploadImage} className='hidden'/>
        </div>
        </label>
        ...
        <button className='bg-red-500 hover:bg-red-600 text-white text-lg font-medium my-2 drop-showdow'>Save</button>
      </form>
    </div>
  )
}
